...
func TestTranslateMetricsGauge(t *testing.T) {
	...
	rm.Resource().Attributes().PutStr("monitoring_compartment_id", "ocid1.compartment.oc1..aaaa")
	rm.Resource().Attributes().PutStr("monitoring_namespace", "otel_demo")
	...
	_, hasNamespace := data[0].Dimensions["monitoring_namespace"]
	_, hasCompartment := data[0].Dimensions["monitoring_compartment_id"]
